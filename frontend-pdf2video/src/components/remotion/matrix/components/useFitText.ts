...
/**
 * Measure-and-shrink font fitter for the Matrix template.
 *
 ...
 * render workspace, so a Matrix layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("matrix-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
