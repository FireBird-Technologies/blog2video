...
/**
 * Measure-and-shrink font fitter for the Sakura template.
 *
 ...
 * render workspace, so a Sakura layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("sakura-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
