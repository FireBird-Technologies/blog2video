...
/**
 * Measure-and-shrink font fitter for the default (Geometric Explainer) template.
 *
 ...
 * default layout importing from `newspaper/` would compile in the Player but
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("default-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
 ...
 * default layouts animate transforms on an ancestor — a projected rect changes
 * every frame and would make the fitted size drift mid-scene, while layout
