...
/**
 * Measure-and-shrink font fitter for the Nightfall template.
 *
 ...
 * render workspace, so a Nightfall layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("nightfall-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
...
/**
 * Scene-wide adapter for templates whose layouts all expose the shared title
 * and description font-size props. It measures the real rendered scene and
 * reduces those two preferred maxima together until no text extends beyond a
 * clipping panel or the video frame. Offset geometry deliberately ignores
 * entrance transforms, so animated copy is fitted to its settled position.
 */
export const withAutoFitLayout = <P extends FittableLayoutProps>(
  ...
  const AutoFitLayout: React.FC<P> = (props) => {
    ...
    const bodyCopies = [props.narration, extra.subtitle, extra.quote, extra.cta, extra.leftLabel, extra.rightLabel, extra.leftDescription, extra.rightDescription, extra.items, extra.metrics, extra.code, extra.codeLines]
      ...
      .filter(Boolean);
    const bodyCopyKey = bodyCopies.join("\u0000");
    const handleRef = React.useRef<number | null>(null);

    React.useLayoutEffect(() => {
      setSizes({ title: props.titleFontSize, description: props.descriptionFontSize });
    }, [props.titleFontSize, props.descriptionFontSize, props.title, bodyCopyKey]);

    React.useLayoutEffect(() => {
      const root = rootRef.current;
      if (!root) return;
      if (handleRef.current === null) handleRef.current = delayRender("nightfall-auto-fit-layout");
      let cancelled = false;
      const release = () => {
        if (handleRef.current !== null) {
          continueRender(handleRef.current);
      ...
      const mirrorOverflows = (kind: "title" | "body") => {
        // Decorative labels and animated artifacts cannot trigger a global
        // resize; only the complete editable-copy mirrors are authoritative.
        const mirrors = Array.from(root.querySelectorAll<HTMLElement>(`[data-auto-fit-mirror="${kind}"]`));
        return mirrors.some((mirror) =>
          mirror.offsetWidth > 0 &&
          mirror.offsetHeight > 0 &&
          (mirror.scrollWidth > mirror.clientWidth + 2 || mirror.scrollHeight > mirror.clientHeight + 2)
        );
      };
      const measure = () => {
