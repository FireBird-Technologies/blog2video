...
/**
 * Measure-and-shrink font fitter for the Economist template.
 *
 ...
 * render workspace, so an Economist layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("economist-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
