...
/**
 * ChronicleDataChart — data-visualization scene (line / bar / histogram) driven
 * by a scraped `chartTable`. Uses the shared data pipeline (_shared/chartData)
 * for all parsing / type inference / animation, so the backend table-binding
 * works unchanged; only the rendering is reskinned into Chronicle's aged
 * parchment aesthetic — an ink-drawn chart on a ledger folio, gold/wax-red
 * series, quill-written title with ornamental corners.
 */

// ─── Chronicle chart palette (ink on parchment) ───────────────────────────────
const INK = "#2A1810";
const CHRON = {
  gold: "#B8860B",
  waxRed: "#8B2E1D",
  sealGreen: "#3F5E3F",
  axisTxt: "rgba(42,24,16,0.82)",
  grid: "rgba(42,24,16,0.14)",
  panelBg: "rgba(248,239,214,0.5)",
  panelBorder: "rgba(42,24,16,0.28)",
} as const;

const DEFAULT_BAR_COLORS = [CHRON.gold, CHRON.waxRed, CHRON.sealGreen] as const;
const VALUE_LABEL_FW = 700;
const AXIS_LINE_STYLE = { stroke: "rgba(42,24,16,0.5)", strokeWidth: 1.4 };
const TICK_LINE_STYLE = { stroke: "rgba(42,24,16,0.4)", strokeWidth: 1.0 };

// Histogram look — tight bins, faint ink edges
const HIST_BIN_RADIUS: [number, number, number, number] = [0, 0, 0, 0];
const HIST_BIN_STROKE = "rgba(42,24,16,0.4)";
...
export const ChronicleDataChart: React.FC<ChronicleLayoutProps> = ({
  title = "The numbers of record",
  narration = "",
  accentColor = CHRON.gold,
  textColor = INK,
  ...
  const ra = Math.min(1, Math.max(0, (frame - 8) / 18));
  ...
  const yTickStyle = { fill: CHRON.axisTxt, fontSize: Math.max(9, chartTickSize), fontWeight: 500, fontFamily: bodyFont };
  const yLabelProp = hasYLabel
    ? {
        ...
        offset: 10,
        style: { fill: CHRON.axisTxt, fontSize: chartAxisLabelSize, fontWeight: 600, textAnchor: "middle" as const, fontFamily: bodyFont },
      }
    : undefined;
  ...
  const buildXCaption = () =>
    xCaptionText
      ? {
          ...
          offset: 8,
          style: { fill: CHRON.axisTxt, fontSize: chartAxisLabelSize, fontWeight: 600, fontFamily: bodyFont, letterSpacing: "0.06em" },
        }
      : undefined;
  ...
  const LegendDot: React.FC<{ color: string; label: string }> = ({ color, label }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ width: 11, height: 11, borderRadius: 2, background: color, border: `1px solid rgba(42,24,16,0.3)` }} />
    ...
    if (resolvedChartType === "line") {
      return (
        <ComposedChart data={linePlotData} margin={{ top: chartMarginTop, right: chartMarginRight, left: chartMarginLeft, bottom: chartBottom }}>
          <defs>
            <linearGradient id="chronicle-line-fill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={barColors[0]} stopOpacity={0.3} />
              <stop offset="95%" stopColor={barColors[0]} stopOpacity={0} />
            </linearGradient>
          </defs>
          <CartesianGrid stroke={CHRON.grid} vertical={false} />
          ...
          <Area
            ...
            fill="url(#chronicle-line-fill)" fillOpacity={0.3} isAnimationActive={false}
            dot={showDots0 ? { r: lineDotR0, fill: "#F8EFD6", stroke: barColors[0], strokeWidth: 1.6 } : false}
            activeDot={false}
          >
            {showDots0 && (
          ...
          {chartInputs.lineSeries[1] && (
            <Line yAxisId={axisForKey("s1")} type="monotone" dataKey="s1"
              ...
              dot={showDots1 ? { r: lineDotR12, fill: "#F8EFD6", stroke: barColors[1], strokeWidth: 1.2 } : false}
          ...
          {chartInputs.lineSeries[2] && (
            <Line yAxisId={axisForKey("s2")} type="monotone" dataKey="s2"
              ...
              dot={showDots2 ? { r: lineDotR12, fill: "#F8EFD6", stroke: barColors[2], strokeWidth: 1.2 } : false}
          ...
          {showDots0 && lastS0 && <ReferenceDot yAxisId={axisForKey("s0")} x={lastS0.x} y={lastS0.y} r={linePointCount >= 16 ? 3 : 4.5} fill="#F8EFD6" stroke={barColors[0]} strokeWidth={2.2} />}
          {showDots1 && lastS1 && <ReferenceDot yAxisId={axisForKey("s1")} x={lastS1.x} y={lastS1.y} r={linePointCount >= 16 ? 2.8 : 3.5} fill="#F8EFD6" stroke={barColors[1]} strokeWidth={1.8} />}
          {showDots2 && lastS2 && <ReferenceDot yAxisId={axisForKey("s2")} x={lastS2.x} y={lastS2.y} r={linePointCount >= 16 ? 2.8 : 3.5} fill="#F8EFD6" stroke={barColors[2]} strokeWidth={1.8} />}
        </ComposedChart>
      );
    }
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: p ? "column" : "row", gap: p ? Math.round(height * 0.02) : Math.round(width * 0.02), minHeight: 0 }}>
          <div
            style={{
              ...
              overflow: "hidden",
              opacity: ra,
              background: CHRON.panelBg,
              border: `1.5px solid ${CHRON.panelBorder}`,
          ...
          >
            {hasRealChart ? (
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: bodyFont, color: "rgba(42,24,16,0.55)", fontSize: descSize, fontStyle: "italic", opacity: ra }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <div
              style={{
                ...
                borderLeft: p ? undefined : `1px dashed rgba(42,24,16,0.25)`,
                ...
                {summaryText
                  ? summaryText.split(/(__[^_]+__)/).map((seg, i) => {
                      if (seg.startsWith("__") && seg.endsWith("__")) {
                        return (
                          <span key={i} style={{ color: CHRON.waxRed, fontWeight: 700 }}>
                            {seg.slice(2, -2)}
                          </span>
                        );
                      }
