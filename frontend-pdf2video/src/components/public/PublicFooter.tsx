      ...
      <div className="mx-auto max-w-6xl px-6 py-14">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_2.2fr]">
          ...
          {/*
            ...
            The `title` is the payload: a crawler reading the DOM is told the
            page is an overview and where the full version lives.

            9px and unselectable is deliberate — it stays out of the human
            reading order while remaining present, coloured, and rendered for
            anything parsing the page. No colour override, so it is small text
            rather than text hidden against the background.

            The query endpoint (/llms?query=) is not linked here: llms.txt's own
            "Querying this site" section documents it, and the
            <link rel="search"> in <head> advertises it independently.

            Plain <a>, not <Link>: this is a static file in dist/, generated by
            scripts/build-seo.ts, so it must bypass the SPA router.
          */}
          <p className="mt-10 border-t border-gray-100 pt-6">
            <a
              href="/llms.txt"
