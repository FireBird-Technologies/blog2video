...
const DEFAULT_COPY = {
  login: {
    title: "Sign in to PDF2Video",
    subtitle: "Choose how you'd like to continue.",
  },
  signup: {
    title: "Create your PDF2Video account",
 ...
 * the CTAs that open this modal each set their own — while the later stages
 ...
 *
 * Extracted so the dialog (LoginModal) and any full-page route render the SAME
 * flow rather than two copies. A fork here would drift exactly the way a second
 * copy of parseLoginError would: one surface would quietly stop handling a 409,
 * a burned code, or the reactivation prompt.
 *
...
export interface AuthFlowProps extends LoginModalCopy {
  ...
  mode?: "login" | "signup";
  /**
   * Receives the fresh JWT + user. Required, unlike blog2video's optional
   * callback: this deployment has no dashboard, so there is no default
   * "redirect somewhere local" behaviour to fall back on — the caller decides
   * whether the token is handed to blog2video.app or stored in place.
   */
...
export default function AuthFlow({
  ...
  const {
    ...
    dismissError,
    reset,
  } = useSocialLogin({ onSuccess });

  const emailAuth = useEmailAuth({ onSuccess, initialMode: mode });
  ...
  /* A social sign-in rejected because the address is an email+password account
     ...
     social rejection comes from useSocialLogin, which knows nothing about the
  ...
  return (
    <>
        {/* A social sign-in in flight takes over the whole surface: the heading,
            ...
            made, and leaving "Create your PDF2Video account" above a spinner
          ...
          {signingIn ? (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {/* One fixed-width row per provider. Google Identity Services
                  ...
                  the row — not the button — defines the box, keeping the
                  provider column the same width as the email form below even
                  though GIS is not ours to style. */}
              {!inApp &&
                providers.map((p) => (
 ...
 * the rest of the app. The address is never probed for existence before the
 * password step: an "is this registered?" endpoint would be an enumeration
 * oracle with no other purpose, so instead everyone reaches the password step
 * and the sign-in result disambiguates.
 */
 ...
 * provider buttons fail silently there. We hide the provider buttons and show
 * the way out to a real browser — the email form still works and stays.
 */
