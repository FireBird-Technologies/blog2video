...
- `editorial_quote`  
  - **Best for**: Standout pull quote,something referenced by someone with peoper reference to wh said what. Something quote or famous saying type effect..
...
- `by_the_numbers`  
  - **Best for**: 2–4 key numeric metrics — statistics, KPIs, or data highlights where every value is a number (e.g. `"4.2B"`, `"98%"`, `"$12B"`). The scene shows a short topic-relevant heading (+ optional standfirst) above a grid of numbers and their short labels — the `value` of every stat must still be a number, never words or sentences. Do not use this layout if the "values" would be words or sentences.  
  - **Rule**: Use `by_the_numbers` **only when this beat actually has content / metrics** — i.e. at least 2 real numeric figures drawn from the source. **If the source has no numeric figures for this beat, do NOT pick `by_the_numbers` — never fabricate figures; use `text_narration`, `feature`, or `editorial_quote` instead.**
...
- `timeline_journey`  
  ...
  - **Rule**: Use `timeline_journey` **only when the beat has at least two events, each with a real date** (year or fuller date drawn from the source). **If the source supplies fewer than two dated events, do NOT pick `timeline_journey` — never fabricate dates; use `text_narration`, `feature`, or `editorial_quote` instead.**
