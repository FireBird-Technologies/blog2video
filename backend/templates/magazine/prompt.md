...
**Props:**
  - `heading` (string) — The headline (last word renders italic). REQUIRED FIELD.
...
**When to Use:** Interviews, debates, two perspectives, or Q&A dialogue. Use `exchanges` with 2–3 full exchanges so the conversation fills the page rather than leaving it sparse. You can also alter the text to display it such that it is in form of question answer pair instead of a single paragraph.
