...
/**
 * Measure-and-shrink font fitter for the Old Documentary Reel template.
 *
 ...
 * render workspace, so a docreel layout importing from another template would
 ...
 *
 * IMPORTANT: give the ref'd element an explicit `width` (e.g. `width: "100%"`
 * alongside `maxWidth`) whenever it sits inside a flex container that does not
 * stretch its children (e.g. `alignItems: "center"`) — otherwise its
 * `clientWidth` shrink-wraps to the current (possibly short) text instead of
 * the true available column, and the measurement below wraps at that
 * artificially narrow width, over-shrinking short copy.
 *
...
export const useFitText = (
   ...
   * against itself can never detect overflow.
   */
  availPx?: number,
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("docreel-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
 ...
 *
 * Uses offsetTop/offsetHeight rather than getBoundingClientRect because
 * several layouts animate a 3D camera on an ancestor — a projected rect
