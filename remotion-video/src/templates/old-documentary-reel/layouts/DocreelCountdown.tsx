...
/**
 * Academy leader countdown — a standalone voiced opening scene.
 *
 * This is a SYSTEM-OWNED scene: the pipeline force-injects it as scene 0 of
 * every documentary video and the LLM never writes it. It carries generic
 * 3-2-1 narration and renders no title/body text. It used to be an 18-frame
...
export const DocreelCountdown: React.FC<SceneLayoutProps> = (props) => {
  ...
  const validCues =
    Array.isArray(countdownCueSeconds) &&
    countdownCueSeconds.length === 3 &&
    countdownCueSeconds.every((cue) => Number.isFinite(Number(cue)))
      ? countdownCueSeconds.map(Number)
      : null;
  const rate = Math.max(0.1, Number(playbackSpeed) || 1);
  // Timestamped TTS drives the visible number. Older scenes without alignment
  // retain the deterministic one-number-per-second fallback.
  const countdownNumber = validCues
    ? frame >= Math.round((validCues[2] * fps) / rate)
      ? 1
      : frame >= Math.round((validCues[1] * fps) / rate)
        ? 2
        : 3
    : Math.max(1, countFrom - Math.floor(frame / TICK_FRAMES));
  const size = p ? width * 0.78 : width * 0.5;
