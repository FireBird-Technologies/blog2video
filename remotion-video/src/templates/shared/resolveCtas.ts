...
/**
 * Reads either the new `ctas` array or legacy single-CTA fields and returns a
 * normalized array (length >= 1). Always returns at least one entry so renderers
 * indexing [0] are safe. Socials are handled separately as a scene-level field.
 */
