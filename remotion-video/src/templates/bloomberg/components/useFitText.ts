...
/**
 * Measure-and-shrink font fitter for the Bloomberg template.
 *
 ...
 * render workspace, so a Bloomberg layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("bloomberg-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
...
/** Real-DOM, scene-wide fitting adapter using the Newspaper shrink contract. */
export const withAutoFitLayout = <P extends FittableLayoutProps>(
  Layout: React.ComponentType<P>,
  defaults: { title: ResponsiveDefault; description: ResponsiveDefault },
): React.FC<P> => {
  const AutoFitLayout: React.FC<P> = (props) => {
    const rootRef = React.useRef<HTMLDivElement>(null);
    const resolveDefault = (value: ResponsiveDefault) => typeof value === "number" ? value : props.aspectRatio === "portrait" ? value.portrait : value.landscape;
    const titleTarget = props.titleFontSize ?? resolveDefault(defaults.title);
    ...
    const bodyCopy = [props.narration, extra.subtitle, extra.quote, extra.cta, extra.leftLabel, extra.rightLabel, extra.leftDescription, extra.rightDescription, extra.items, extra.metrics, extra.code]
      ...
      .join("\n");
    const handleRef = React.useRef<number | null>(null);
    React.useLayoutEffect(() => setSizes({ title: props.titleFontSize, description: props.descriptionFontSize }), [props.titleFontSize, props.descriptionFontSize]);
    React.useLayoutEffect(() => {
      const root = rootRef.current;
      if (!root) return;
      if (handleRef.current === null) handleRef.current = delayRender("bloomberg-auto-fit-layout");
      let cancelled = false;
      const release = () => {
        if (handleRef.current !== null) {
          continueRender(handleRef.current);
      ...
      // Only editable-copy mirrors may trigger a resize. Chart annotations,
      // decorative HUD labels and entrance artifacts are intentionally dense
      // and must never make otherwise short copy globally smaller.
      const mirrorOverflows = (kind: "title" | "body") => {
        const mirror = root.querySelector<HTMLElement>(`[data-auto-fit-mirror="${kind}"]`);
        if (!mirror || mirror.offsetWidth === 0 || mirror.offsetHeight === 0) return false;
        return mirror.scrollWidth > mirror.clientWidth + 2 || mirror.scrollHeight > mirror.clientHeight + 2;
      };
      const measure = () => {
    ...
    }, [sizes, titleTarget, descriptionTarget, props.title, bodyCopy]);
    return React.createElement(
      ...
      bodyCopy ? React.createElement("div", { "data-auto-fit-mirror": "body", style: { position: "absolute", visibility: "hidden", width: "82%", height: "30%", overflow: "hidden", fontSize: sizes.description ?? descriptionTarget, lineHeight: 1.45, whiteSpace: "pre-wrap", overflowWrap: "anywhere" } }, bodyCopy) : null,
