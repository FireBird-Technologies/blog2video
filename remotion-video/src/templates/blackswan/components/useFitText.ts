...
/**
 * Measure-and-shrink font fitter for the Black Swan template.
 *
 ...
 * render workspace, so a Black Swan layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("blackswan-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
