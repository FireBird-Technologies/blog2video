 ...
 * parsing / type inference / animation; rendered in the Black Swan neon aesthetic
 * — cyan-on-black glass panel, glowing series, starfield ambience.
 */
...
export const BlackswanDataViz: React.FC<BlackswanLayoutProps> = ({
  title = "Signal analysis",
  narration = "The data surfaces the pattern beneath the noise.",
  accentColor = "#00E5FF",
  bgColor = "#000000",
  textColor = "#DFFFFF",
  ...
  const palette = useMemo(() => blackswanNeonPalette(accentColor), [accentColor]);
  const tube = useMemo(() => neonTitleTubeStyle(accentColor, { bgColor }), [accentColor, bgColor]);

  // Neon series: accent + two ladder hues derived from the accent, all overridable.
  const SERIES_2 = palette.vivid;
  const SERIES_3 = palette.deep;
  const DEFAULT_BAR_COLORS = [accentColor, SERIES_2, SERIES_3] as const;
  const axisInk = accentColor;
  const GRID_INK = rgbaFromHex(accentColor, 0.16);
  ...
  const AXIS_LINE_STYLE = { stroke: axisInk, strokeWidth: 1.4, strokeOpacity: 0.5 };
  const TICK_LINE_STYLE = { stroke: axisInk, strokeWidth: 1.0, strokeOpacity: 0.45 };
  ...
  const buildXCaption = () =>
    xCaptionText
      ? {
          ...
          offset: 8,
          style: { fill: axisInk, fontSize: chartAxisLabelSize, fontWeight: 400, fontFamily: font, letterSpacing: "0.06em" },
        }
      : undefined;
  ...
  const renderChart = () => {
    if (resolvedChartType === "line") {
      return (
        <ComposedChart data={linePlotData} margin={{ top: chartMarginTop, right: chartMarginRight, left: chartMarginLeft, bottom: chartBottom }}>
          <defs>
            <linearGradient id="bsw-line-fill" x1="0" y1="0" x2="0" y2="1">
          ...
          <Area
            ...
            fill="url(#bsw-line-fill)" fillOpacity={0.28} isAnimationActive={false}
            dot={showDots0 && !useLineEndCallouts ? { r: lineDotR0, fill: bgColor, stroke: barColors[0], strokeWidth: 1.4 } : false}
            activeDot={false}
          >
          ...
          {chartInputs.lineSeries[1] && (
            <Line yAxisId={axisForKey("s1")} type="monotone" dataKey="s1"
              ...
              dot={showDots1 && !useLineEndCallouts ? { r: lineDotR12, fill: bgColor, stroke: barColors[1], strokeWidth: 1.1 } : false}
          ...
          {chartInputs.lineSeries[2] && (
            <Line yAxisId={axisForKey("s2")} type="monotone" dataKey="s2"
              ...
              dot={showDots2 && !useLineEndCallouts ? { r: lineDotR12, fill: bgColor, stroke: barColors[2], strokeWidth: 1.1 } : false}
          ...
          {showDots0 && !useLineEndCallouts && lastS0 && <ReferenceDot yAxisId={axisForKey("s0")} x={lastS0.x} y={lastS0.y} r={linePointCount >= 16 ? 3 : 4} fill={bgColor} stroke={barColors[0]} strokeWidth={2} />}
    ...
    if (resolvedChartType === "histogram") {
      const histBarStroke = { stroke: rgbaFromHex(accentColor, 0.55), strokeWidth: HIST_BIN_STROKE_W };
      ...
      return (
        <BarChart data={animatedHistData}
          ...
          <YAxis axisLine={AXIS_LINE_STYLE} tickLine={TICK_LINE_STYLE} tick={yTickStyle} width={yAxisWidth}
            domain={customYAxis ? customYAxis.domain : [0, histAxisTop]} ticks={customYAxis?.ticks} tickFormatter={yAxisTickFmt} label={yLabelProp} />
          <Bar dataKey="value" fill={defaultBarColor} radius={HIST_BIN_RADIUS} maxBarSize={histMaxSingleScaled} isAnimationActive={false} stroke={rgbaFromHex(accentColor, 0.55)} strokeWidth={HIST_BIN_STROKE_W}>
  ...
  const LegendDot: React.FC<{ color: string; label: string }> = ({ color, label }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span style={{ color: accentColor, fontSize: 14, fontWeight: 500, fontFamily: font }}>{label}</span>
    </div>
  );
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: p ? "column" : "row", gap: p ? Math.round(height * 0.02) : Math.round(width * 0.02), minHeight: 0 }}>
          <div
            style={{
              ...
              boxShadow: `0 0 30px ${rgbaFromHex(accentColor, 0.18)}, inset 0 0 44px ${rgbaFromHex(accentColor, 0.05)}`,
            }}
          >
            {hasRealChart ? (
              <div
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <div
              style={{
                ...
                borderTop: `1px solid ${rgbaFromHex(accentColor, 0.27)}`,
                borderLeft: p ? undefined : `1px solid ${rgbaFromHex(accentColor, 0.13)}`,
