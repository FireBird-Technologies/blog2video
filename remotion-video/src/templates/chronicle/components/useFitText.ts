...
/**
 * Measure-and-shrink font fitter for the Chronicle template.
 *
 ...
 * render workspace, so a Chronicle layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("chronicle-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
