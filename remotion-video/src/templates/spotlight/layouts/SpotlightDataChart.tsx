 ...
 * a scraped `chartTable`. Uses the shared data pipeline (_shared/chartData) for all
 * parsing / type inference / animation, so the backend table-binding works
 * unchanged; only the rendering is reskinned into Spotlight's bold, high-contrast
 * aesthetic — a glass-panel chart card on a black stage, white text, accent-red
 * series, Archivo Black title with a spring-slam entrance.
 */

// ─── Spotlight chart palette ──────────────────────────────────────────────────
const SPOT = {
  accent: "#EF4444",
  accentDim: "#B91C1C",
  accentSoft: "#F87171",
  axisTxt: "rgba(255,255,255,0.78)",
...
const DEFAULT_BAR_COLORS = [SPOT.accent, SPOT.accentSoft, "#FCA5A5"] as const;
const VALUE_LABEL_FW = 700;
const AXIS_LINE_STYLE = { stroke: "rgba(255,255,255,0.35)", strokeWidth: 1.4 };
const TICK_LINE_STYLE = { stroke: "rgba(255,255,255,0.30)", strokeWidth: 1.0 };

// Histogram look — tight bins, faint edges
const HIST_BIN_RADIUS: [number, number, number, number] = [0, 0, 0, 0];
const HIST_BIN_STROKE = "rgba(0,0,0,0.35)";
...
export const SpotlightDataChart: React.FC<SpotlightLayoutProps> = ({
  title = "By the numbers",
  narration = "",
  accentColor = SPOT.accent,
  bgColor = "#000000",
  textColor = SPOT.white,
  ...
  const accent = accentColor || SPOT.accent;

  const titleSize = titleFontSize ?? (p ? 64 : 58);
  const descSize = descriptionFontSize ?? (p ? 30 : 28);
  ...
  const buildXAxisProps = (
    ...
    buildXAxisPropsShared(labels, isPortrait, descSizeArg, forceAllLabels, chartAreaPx, {
      ...
      axisTextColor: SPOT.axisTxt,
      fontFamily: bodyFont,
    });

  // Title spring-slam (1.18 over-scale → 1.0) + panel reveal.
  const titleSlam = spring({ frame: frame - 1, fps, config: { damping: 18, stiffness: 180 } });
  const titleScale = 1.18 - 0.18 * titleSlam;
  const titleOpacity = Math.min(1, Math.max(0, frame / 6));
  const ra = Math.min(1, Math.max(0, (frame - 4) / 16));
  ...
  const yTickStyle = { fill: SPOT.axisTxt, fontSize: Math.max(9, chartTickSize), fontWeight: 500, fontFamily: bodyFont };
  const yLabelProp = hasYLabel
    ? {
        ...
        offset: 10,
        style: { fill: SPOT.axisTxt, fontSize: chartAxisLabelSize, fontWeight: 600, textAnchor: "middle" as const, fontFamily: bodyFont },
      }
    : undefined;
  ...
  const buildXCaption = () =>
    xCaptionText
      ? {
          ...
          offset: 8,
          style: { fill: SPOT.axisTxt, fontSize: chartAxisLabelSize, fontWeight: 600, fontFamily: bodyFont, letterSpacing: "0.06em" },
        }
      : undefined;
  ...
  const LegendDot: React.FC<{ color: string; label: string }> = ({ color, label }) => (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span style={{ color: SPOT.white, fontSize: 14, fontWeight: 700, fontFamily: bodyFont, opacity: 0.92 }}>
        {label}
      </span>
    </div>
  );
    ...
    if (resolvedChartType === "line") {
      return (
        <ComposedChart data={linePlotData} margin={{ top: chartMarginTop, right: chartMarginRight, left: chartMarginLeft, bottom: chartBottom }}>
          <defs>
            <linearGradient id="spotlight-line-fill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={barColors[0]} stopOpacity={0.32} />
              <stop offset="95%" stopColor={barColors[0]} stopOpacity={0} />
            </linearGradient>
          </defs>
          <CartesianGrid stroke={SPOT.grid} vertical={false} />
          ...
          <Area
            ...
            fill="url(#spotlight-line-fill)" fillOpacity={0.3} isAnimationActive={false}
            dot={showDots0 ? { r: lineDotR0, fill: SPOT.white, stroke: barColors[0], strokeWidth: 1.6 } : false}
            activeDot={false}
          >
            {showDots0 && (
              <LabelList dataKey="s0" position="top" offset={8}
                formatter={(v) => formatLineLabel(v as number | null | undefined)}
                fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} />
            )}
          </Area>
          ...
          {chartInputs.lineSeries[1] && (
            <Line yAxisId={axisForKey("s1")} type="monotone" dataKey="s1"
              ...
              dot={showDots1 ? { r: lineDotR12, fill: SPOT.white, stroke: barColors[1], strokeWidth: 1.2 } : false}
          ...
          {chartInputs.lineSeries[2] && (
            <Line yAxisId={axisForKey("s2")} type="monotone" dataKey="s2"
              ...
              dot={showDots2 ? { r: lineDotR12, fill: SPOT.white, stroke: barColors[2], strokeWidth: 1.2 } : false}
          ...
          {showDots0 && lastS0 && <ReferenceDot yAxisId={axisForKey("s0")} x={lastS0.x} y={lastS0.y} r={linePointCount >= 16 ? 3 : 4.5} fill={SPOT.white} stroke={barColors[0]} strokeWidth={2.2} />}
          {showDots1 && lastS1 && <ReferenceDot yAxisId={axisForKey("s1")} x={lastS1.x} y={lastS1.y} r={linePointCount >= 16 ? 2.8 : 3.5} fill={SPOT.white} stroke={barColors[1]} strokeWidth={1.8} />}
          {showDots2 && lastS2 && <ReferenceDot yAxisId={axisForKey("s2")} x={lastS2.x} y={lastS2.y} r={linePointCount >= 16 ? 2.8 : 3.5} fill={SPOT.white} stroke={barColors[2]} strokeWidth={1.8} />}
        </ComposedChart>
      );
    }
        ...
        return (
          <BarChart data={animatedCompBarData as BarDatum[]}
            ...
            <Bar dataKey="s0" fill={barColors[0]} radius={HIST_BIN_RADIUS} maxBarSize={histMaxGroupedScaled} isAnimationActive={false} {...histBarStroke}>
              {showHistTopLabels ? <LabelList dataKey="s0" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
            </Bar>
            {chartInputs.lineSeries[1] && (
              <Bar dataKey="s1" fill={barColors[1]} radius={HIST_BIN_RADIUS} maxBarSize={histMaxGroupedScaled} isAnimationActive={false} {...histBarStroke}>
                {showHistTopLabels ? <LabelList dataKey="s1" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
              </Bar>
            )}
            {chartInputs.lineSeries[2] && (
              <Bar dataKey="s2" fill={barColors[2]} radius={HIST_BIN_RADIUS} maxBarSize={histMaxGroupedScaled} isAnimationActive={false} {...histBarStroke}>
                {showHistTopLabels ? <LabelList dataKey="s2" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
              </Bar>
            )}
          </BarChart>
        );
      ...
      return (
        <BarChart data={animatedHistData}
          ...
          <Bar dataKey="value" fill={defaultBarColor} radius={HIST_BIN_RADIUS} maxBarSize={histMaxSingleScaled} isAnimationActive={false} stroke={HIST_BIN_STROKE} strokeWidth={HIST_BIN_STROKE_W}>
            {showHistTopLabels ? <LabelList dataKey="value" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
          </Bar>
        </BarChart>
      );
    }
        ...
        {hasComparisonBars ? (
          <>
            <Bar dataKey="s0" fill={barColors[0]} radius={[3, 3, 0, 0]} maxBarSize={barMaxSizeGrouped} isAnimationActive={false}>
              {showBarTopValueLabels ? <LabelList dataKey="s0" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
            </Bar>
            {chartInputs.lineSeries[1] && (
              <Bar dataKey="s1" fill={barColors[1]} radius={[3, 3, 0, 0]} maxBarSize={barMaxSizeGrouped} isAnimationActive={false}>
                {showBarTopValueLabels ? <LabelList dataKey="s1" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
              </Bar>
            )}
            {chartInputs.lineSeries[2] && (
              <Bar dataKey="s2" fill={barColors[2]} radius={[3, 3, 0, 0]} maxBarSize={barMaxSizeGrouped} isAnimationActive={false}>
                {showBarTopValueLabels ? <LabelList dataKey="s2" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
              </Bar>
            )}
          </>
        ) : (
          <Bar dataKey="value" fill={defaultBarColor} radius={[4, 4, 0, 0]} maxBarSize={barMaxSizeSingle} isAnimationActive={false}>
            {showBarTopValueLabels ? <LabelList dataKey="value" position="top" offset={10} fill={SPOT.white} fontSize={VALUE_LABEL_FS} fontWeight={VALUE_LABEL_FW} fontFamily={bodyFont} formatter={(v) => formatBarLabel(v, useCompact)} /> : null}
          </Bar>
        )}
      </BarChart>
    );
    ...
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: bgColor }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", padding: `${padV} ${padH}` }}>
        ...
        <div style={{ opacity: titleOpacity, marginBottom: Math.round(height * 0.022), transform: `scale(${titleScale})`, transformOrigin: "left center" }}>
          ...
          <div style={{ width: 84, height: 6, background: accent, marginTop: Math.round(height * 0.012), borderRadius: 2 }} />
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: p ? "column" : "row", gap: p ? Math.round(height * 0.02) : Math.round(width * 0.02), minHeight: 0 }}>
          <div
            style={{
              ...
              overflow: "hidden",
              opacity: ra,
              background: SPOT.panelBg,
              backdropFilter: "blur(16px)",
              WebkitBackdropFilter: "blur(16px)",
              border: `1px solid ${SPOT.panelBorder}`,
          ...
          >
            {hasRealChart ? (
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: bodyFont, color: "rgba(255,255,255,0.55)", fontSize: descSize, fontStyle: "italic", opacity: ra }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <div
              style={{
                opacity: Math.min(1, Math.max(0, (frame - 12) / 20)),
                borderTop: `2px solid ${accent}`,
                borderLeft: p ? undefined : `1px solid rgba(255,255,255,0.10)`,
                paddingTop: Math.round(descSize * 0.5),
                paddingLeft: p ? 0 : Math.round(descSize * 0.4),
                overflow: "hidden",
              }}
            >
              <div style={{ fontFamily: bodyFont, fontWeight: 600, fontSize: descSize * 0.86, lineHeight: 1.5, color: textColor || SPOT.white, opacity: 0.95, whiteSpace: "pre-wrap" }}>
                {summaryText
