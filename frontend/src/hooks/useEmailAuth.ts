 ...
 *
 * Mirrors useSocialLogin's contract (same onSuccess/skipRedirect options, the
 * same LoginError shape, the same held-credential reactivation retry) so the
 * modal can treat both paths identically and the 409 wrong-provider recovery
 * works the same whichever one produced it.
 */
...
interface UseEmailAuthOptions {
   ...
   *  click from the "Already have an account? Sign in" toggle. /signin passes
   *  "login" explicitly so that route still opens on the sign-in form. */
