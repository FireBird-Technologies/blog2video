...
import { getTemplates, getVoicePreviews, type TemplateMeta, type VoicePreview } from "../api/client";
...
function StepIndicator({ current, total }: { current: number; total: number }) {
  const stepLabels = ["Content", "Voice", "Template"];
  return (
...
export default function BlogUrlForm({ onSubmit, loading, asModal, onClose }: Props) {
        ...
        <div>
          ...
          {docFiles.length > 0 && (
            <div className="mt-3 space-y-1.5">
              {docFiles.map((file, i) => (
                <div key={`${file.name}-${i}`} className="flex items-center gap-2 px-3 py-2 bg-gray-50/80 rounded-lg">
                  <svg
                    className={`w-4 h-4 flex-shrink-0 ${file.name.endsWith(".pdf") ? "text-red-400" : file.name.endsWith(".docx") ? "text-blue-400" : "text-orange-400"}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                  ...
                  <button type="button" onClick={() => removeDocFile(i)} className="text-gray-300 hover:text-red-500 transition-colors flex-shrink-0">
  ...
  // ─── Step 2: Voice ───────────────────────────────────────────
  // Sort so voices with preview_url come first — voices without are shown last
  const voiceOptions = [
    ...
    const aHas = voicePreviews[a.key]?.preview_url ? 1 : 0;
    const bHas = voicePreviews[b.key]?.preview_url ? 1 : 0;
    return bHas - aHas; // voices with preview first
  });
      ...
      {voiceGender !== "none" && (
        <div>
          <label className="block text-[11px] font-medium text-gray-400 mb-3 uppercase tracking-wider">
            Select a voice — click play to preview
          </label>
          <div className="grid grid-cols-2 gap-3">
            {voiceOptions.map(({ gender, accent, key, flag }) => {
              ...
              return (
                <div
                  key={key}
                  onClick={() => { setVoiceGender(gender); setVoiceAccent(accent); }}
                ...
                >
                  ...
                  <div className="flex items-start gap-2.5 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-gray-800">
                        {preview?.name ?? (gender === "female" ? (accent === "american" ? "Rachel" : "Alice") : accent === "american" ? "Bill" : "Daniel")}
                      </div>
                      <div className="text-[10px] text-gray-400 mt-0.5">
                        {gender === "female" ? "Female" : "Male"} • {accent === "american" ? "American" : "British"}
                      </div>
                      {preview?.description && (
                        <div className="text-[10px] text-gray-400 mt-0.5 italic">{preview.description}</div>
                      )}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      playVoice(key, preview?.preview_url ?? null);
                    }}
                  ...
                  >
                    {isPlaying ? (
                      ...
                      <>
                        ...
                        {preview?.preview_url ? "Preview" : "No preview"}
                      </>
                    )}
                  </button>
                </div>
              ...
              >
                ...
                <div className="relative overflow-hidden">
                  {PreviewComp ? (
                    ...
                    <div className="w-full aspect-video bg-gray-100 flex items-center justify-center text-gray-300 text-xs">
                      {t.name}
                    </div>
                  )}
                  {/* Expand-to-video button — appears on hover */}
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setVideoPreviewId(t.id); }}
                  ...
                  >
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/90 text-gray-800 text-[11px] font-medium shadow-sm">
                ...
                {/* Info */}
                <div className={`px-3 py-2.5 transition-colors ${isSelected ? "bg-purple-50/80" : "bg-white/80"}`}>
                  <div className="text-xs font-semibold text-gray-800">
                    {desc?.title ?? t.name}
                  </div>
                  {desc?.subtitle && (
                    <div className="text-[10px] text-gray-400 mt-0.5">{desc.subtitle}</div>
                  )}
                </div>
              </div>
            );
          ...
          <div className="mt-2">
            ...
            <div className="mt-2.5">
              <label className="block text-[10px] text-gray-400 mb-1">
                Opacity <span className="text-gray-300">{Math.round(logoOpacity * 100)}%</span>
              </label>
              <input
                type="range"
                min={10}
  ...
  // ─── Render ──────────────────────────────────────────────────
  const stepContent = step === 1 ? step1 : step === 2 ? step2 : step3;

  // Modal max-width: wider on step 3 for side-by-side template previews
