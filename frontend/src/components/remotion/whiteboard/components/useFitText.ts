...
/**
 * Measure-and-shrink font fitter for the Whiteboard template.
 *
 ...
 * render workspace, so a Whiteboard layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("whiteboard-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
