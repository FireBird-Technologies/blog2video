...
/**
 * NewsTimeline — chronological events list.
 *
 * Animation (30 fps):
 *  0-16   title + rule fade in
 *  12-26  vertical spine draws downward
 *  Item reveal stagger (each ~14f apart, starting at frame 18):
 *    - dot pops in
 *    - date fades (simultaneous)
 *    - text slides right
 *
 * Props:
 *  title        = timeline header
 *  narration    = optional caption
 *  stats[]      = up to 5 events: { value: "Jan 31 2026", label: "Event description" }
 *  accentColor  = dot + highlight color
 */
        ...
        <div style={{ flex: 1, display: "flex", position: "relative" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: p ? 14 : 18 }}>
            {items.map((item, i) => {
              ...
              // Highlight the last item (most recent/current) with accent
              ...
              return (
                ...
                >
                  {/* Dot — positioned over the spine */}
                  <div
                    style={{
                      position: "absolute",
                      left: -(p ? 18 : 28) - 1 - 6, // spine offset - 1px (border) - dot radius
                      top: p ? 4 : 5,
                      width: 12,
                      height: 12,
                      borderRadius: "50%",
                      ...
                      boxShadow: isLatest ? `0 0 0 3px ${accentColor}44` : "none",
                    }}
                  />
                  ...
                  <div
                    style={{
                      ...
                      opacity: dateOp * (isLatest ? 1 : 0.55),
                      whiteSpace: "nowrap",
                      minWidth: p ? 52 : 62,
                      letterSpacing: "0.04em",
                      marginTop: p ? 2 : 3,
                      filter: isLatest ? `drop-shadow(0 0 4px ${accentColor})` : "none",
                    }}
                  >
                    {item.value}
                  </div>
                  ...
                  <div
                    style={{
                      fontFamily: B_FONT,
                      fontSize: descriptionFontSize ?? (p ? 15 : 18),
                      color: textColor,
