...
export const TerminalDataViz: React.FC<BloombergLayoutProps> = ({
  ...
  // lineColors[i] is the definitive color for series i — used in both chart and legend
  ...
  // Reserve space below for x-tick labels + optional x-title
  ...
  // chartHeight = SVG chart area only; ChartRenderer adds xReserve below
  const chartHeight = p
      ...
      {labels.map((lbl, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            left: cx,
            top: 8,
            transform: angle !== 0 ? `translateX(-50%) rotate(${angle}deg)` : "translateX(-50%)",
