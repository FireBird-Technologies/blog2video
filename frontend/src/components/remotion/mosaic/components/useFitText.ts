...
/**
 * Measure-and-shrink font fitter for the Mosaic template.
 *
 ...
 * render workspace, so a Mosaic layout importing from `magazine/` would
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("mosaic-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
