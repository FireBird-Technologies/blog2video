...
/**
 * Measure-and-shrink font fitter for the spotlight template.
 *
 ...
 * spotlight layout importing from `newspaper/` would compile in the Player but
...
export const useFitText = (
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("spotlight-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
 ...
 *
 * Uses offsetTop/offsetHeight rather than getBoundingClientRect because some
 * spotlight layouts animate transforms on an ancestor — a projected rect
