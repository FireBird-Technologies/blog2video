...
import type { NewscastLayoutProps } from "./layouts/types";
import {
...
> = ({
  ...
  // Blinking LIVE dot (about 1s period)
  ...
  const safeTicker = useMemo(() => {
    ...
    return items.length ? items : ["LIVE BREAKING FEED", "LATEST UPDATES", "OFFICIAL CONFIRMATIONS"];
  }, [tickerItems]);

  // Loop marquee based on frames so it matches rendering time deterministically.
  const loopFrames = Math.max(30, Math.round(10 * fps));
  const progress = ((frame % loopFrames) / loopFrames) * 100; // 0..100

  const gapPx = width < 900 ? 22 : 32;
  const sep = "◆";
      ...
      >
        ...
        >
          ...
          >
            ...
            {[...safeTicker, ...safeTicker].flatMap((txt, idx) => {
              const showSep = idx !== 0;
              return (
                <React.Fragment key={`${txt}-${idx}`}>
                  {showSep ? (
                    <span style={{ color: RED, fontWeight: 700, fontSize: display12, padding: "0 6px" }}>{sep}</span>
                  ) : null}
                  <span>{txt}</span>
                </React.Fragment>
              );
