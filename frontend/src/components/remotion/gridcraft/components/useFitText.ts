...
/**
 * Measure-and-shrink font fitter for the gridcraft template.
 *
 ...
 * being clipped by a bento cell's `overflow:hidden`.
 *
 ...
 * MP4 match the live Player.
 *
 * Single-column measure-and-shrink fitter (ported from the newspaper template).
 * Deliberately duplicated rather than imported: the backend copies only
 * `src/templates/{template_id}/` into each per-project render workspace, so a
 * gridcraft layout importing from `newspaper/` would compile in the Player but
 * break the headless render with an unresolved import.
 *
 ...
 * element (body fits its budget first, then the give-back). Pass
 * `minPx === targetPx` to make the hook a no-op while still calling it
...
export const useFitText = (
   ...
   *
   * Needed for elements that are NOT allowed to shrink (`flex-shrink:0`), or
   * that sit inside a fixed-height bento cell whose clientHeight always equals
   * its content height — measuring such an element against itself can never
   * detect overflow.
   */
  availPx?: number,
  ...
  React.useLayoutEffect(() => {
    ...
    if (handleRef.current === null) {
      handleRef.current = delayRender("gridcraft-fit-text");
    }
    let cancelled = false;
    const release = () => {
      if (handleRef.current !== null) {
 ...
 *
 * Uses offsetTop/offsetHeight rather than getBoundingClientRect because a
 * projected rect would change every frame under an animated transform, while
