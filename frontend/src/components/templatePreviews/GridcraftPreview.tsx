...
function useStagger(count: number, delayMs: number, active: boolean) {
  ...
  useEffect(() => {
    ...
    const timers = Array.from({ length: count }, (_, i) =>
      setTimeout(() => setVis((v) => [...v, i]), i * delayMs + 150)
    );
...
function Blobs() {
  return (
    <>
      <div style={{ position: "absolute", width: 280, height: 280, borderRadius: "50%", background: "radial-gradient(circle, rgba(249,115,22,0.28) 0%, transparent 70%)", top: "-20%", left: "-10%", filter: "blur(50px)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", width: 220, height: 220, borderRadius: "50%", background: "radial-gradient(circle, rgba(251,191,36,0.18) 0%, transparent 70%)", bottom: "-15%", right: "-5%", filter: "blur(50px)", pointerEvents: "none" }} />
    </>
  );
}
...
function SlideBentoHero({ active }: { active: boolean }) {
  const vis = useStagger(3, 100, active);
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: BG, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Blobs />
      <div style={{ width: "86%", height: "74%", position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "2fr 1fr", gridTemplateRows: "1fr 1fr", gap: 8 }}>
        <div style={{ ...glassStyle(true), gridRow: "1 / 3", display: "flex", flexDirection: "column", justifyContent: "flex-end", ...pop(0) }}>
          <div style={{ fontSize: 9, letterSpacing: "0.12em", textTransform: "uppercase" as const, opacity: 0.75, marginBottom: 6, fontWeight: 300 }}>Template System</div>
          <div style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.1, fontFamily: "Georgia, serif" }}>Gridcraft</div>
          <div style={{ fontSize: 11, opacity: 0.8, marginTop: 5, fontWeight: 300 }}>Bento editorial in motion</div>
        </div>
        ...
        <div style={{ ...glassStyle(false), display: "flex", flexDirection: "column", justifyContent: "center", ...pop(2) }}>
          <div style={{ fontSize: 9, color: MUTED, fontWeight: 300 }}>Version</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: DARK }}>1.0</div>
        </div>
      </div>
    </div>
  );
...
function SlideBentoFeatures({ active }: { active: boolean }) {
  const vis = useStagger(6, 80, active);
  return (
    <div style={{ width: "100%", height: "100%", background: BG, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Blobs />
      <div style={{ width: "86%", height: "80%", position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gridTemplateRows: "1fr 1fr", gap: 8 }}>
        {FEATURES.map((f, i) => (
          <div key={i} style={{
            ...glassStyle(i === 2),
            opacity: vis.includes(i) ? 1 : 0,
...
function KpiCell({ num, suffix, label, accent, delay, active }: { num: number; suffix: string; label: string; accent: boolean; delay: number; active: boolean }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 36, fontWeight: 700, lineHeight: 1, fontVariantNumeric: "tabular-nums" as const }}>
        {counter}{suffix}
      </div>
      <div style={{ fontSize: 10, opacity: accent ? 0.8 : undefined, color: !accent ? MUTED : undefined, textTransform: "uppercase" as const, letterSpacing: "0.08em", fontWeight: 300 }}>
        {label}
      </div>
    </div>
  );
...
function SlideKpiGrid({ active }: { active: boolean }) {
  return (
    <div style={{ width: "100%", height: "100%", background: BG, position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Blobs />
      <div style={{ width: "86%", height: "60%", position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {KPIS.map((kpi, i) => <KpiCell key={i} {...kpi} delay={i * 120 + 100} active={active} />)}
      </div>
    </div>
  );
}
