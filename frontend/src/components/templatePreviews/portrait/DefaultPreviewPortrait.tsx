...
// ── Preview palette (matches DefaultPreview)
const ACCENT = "#007AFF";
const BG     = "#F5F5F7";
const TEXT   = "#1D1D1F";
const YELLOW = "#FFD60A";
...
// ── SLIDE 1 — Hero (orbiting plane around stacked title)
function SlideHeroImage({ active }: { active: boolean }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: BG, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", overflow: "hidden", position: "relative", fontFamily: "sans-serif" }}>
      <div style={{
        position: "absolute",
        zIndex: 10,
      ...
      <div style={{ textAlign: "center", zIndex: 1, maxWidth: "80%", padding: "0 18px" }}>
        <h1 style={{ fontSize: 40, lineHeight: 1.05, fontWeight: 900, color: TEXT, margin: 0, textTransform: "uppercase", opacity: lerp(titleS, 0, 1), transform: `scale(${lerp(titleS, 0.8, 1)})` }}>
          Launch<br />Ready
        </h1>
        <p style={{ fontSize: 13, color: TEXT, opacity: lerp(narrationS, 0, 0.7), marginTop: 16, lineHeight: 1.5, transform: `translateY(${lerp(narrationS, 10, 0)}px)` }}>
...
// ── SLIDE 2 — Text narration that blows apart
...
function SlideTextNarration({ active }: { active: boolean }) {
  ...
  const corners = useMemo(() => {
    return Array.from({ length: 4 }).map(() =>
      Array.from({ length: 4 }).map((_, i) => {
        ...
        return { x, width, angle, color: colors[i % colors.length] };
      })
    );
  }, []);
  ...
  const getBlowProps = (seed: number) => {
    ...
    const dist = 120 + Math.abs(Math.cos(seed) * 160);
    return { x: Math.cos(angle) * dist, y: Math.sin(angle) * dist, rotate: Math.sin(seed * 3) * 540 };
  };

  return (
    <div style={{ width: "100%", height: "100%", background: BG, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative", fontFamily: "sans-serif" }}>
      {[
      ...
      <div style={{ textAlign: "center", maxWidth: "82%", display: "flex", flexWrap: "wrap", justifyContent: "center" }}>
        {words_array.map((word, wi) => (
          <div key={wi} style={{ display: "flex", marginRight: 8 }}>
...
function SlideMetric({ active }: { active: boolean }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 28, fontFamily: "sans-serif" }}>
      <div style={{ position: "relative", width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width={size} height={size} style={{ transform: "rotate(-90deg)", position: "absolute" }}>
          <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={`${TEXT}11`} strokeWidth={14} />
          <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={ACCENT} strokeWidth={14} strokeDasharray={circ} strokeDashoffset={offset} strokeLinecap="round" />
        </svg>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 52, fontWeight: 900, color: TEXT }}>{count}%</div>
          <div style={{ fontSize: 10, fontWeight: 700, color: ACCENT, letterSpacing: 2 }}>RETENTION</div>
        </div>
      </div>
      <p style={{ fontSize: 13, color: `${TEXT}99`, maxWidth: "75%", textAlign: "center", lineHeight: 1.5, margin: 0 }}>
        Viewers stay engaged to the end.
      </p>
    </div>
  );
}

export default function DefaultPreviewPortrait({ thumbnailMode = false }: { thumbnailMode?: boolean } = {}) {
